...
class NormalizingFlow(PyroModule):
    ...
    def __init__(self, flow_type, bounds, *flow_maker_args, embedding_net = None, 
                 device=None, **flow_maker_kwargs):
        super().__init__()
        ...
        self.base_dist = dist.Normal(
            torch.zeros(flow_maker_args[0], device=device),
            torch.ones(flow_maker_args[0], device=device))
    ...
    def log_prob(self, x, *args, condition = None, **kwargs):
        ...
        '''
        with nvtx.range("log_prob"):
            if self.bounds is None:
                y, log_jac = x, torch.tensor(0., device=x.device, dtype=x.dtype)
            else:
                y, log_jac = bounding_transform(x, self.bounds['low'], self.bounds['high'])
            if self.conditional:
                assert condition is not None
                emb = self.embedding_net(condition)
    ...
    def bounded_log_prob(self, x, *args, condition = None, **kwargs):
        ...
        lp = torch.log(torch.zeros_like(x[:,0], device=x.device, dtype=x.dtype))
