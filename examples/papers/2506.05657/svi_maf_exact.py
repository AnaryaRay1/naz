...
from naz.flows.bflow_jax_maf import make_conditional_autoregressive_nn, make_masked_affine_autoregressive_transform, make_normalizing_flow, train_maf, bayesian_normalizing_flow, train_bayesian_flow_hmc, train_bayesian_flow_prior, train_bayesian_flow, torch_to_jax, train_bayesian_flow_svi
...
posterior_samples, svi_params = train_bayesian_flow_svi(model, guide, unravel_fn, scale_max = sm, num_samples=ns, num_steps = 7000)

with open(f"__run__/bayesian_flow_svi_{out}.pkl", "wb") as pf:
    pickle.dump({"posterior":posterior_samples, "svi_params":svi_params}, pf)
