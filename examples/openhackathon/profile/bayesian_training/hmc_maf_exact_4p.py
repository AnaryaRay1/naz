...
model, guide, guided_model, unravel_fn = bayesian_normalizing_flow(flow["lp"], best_params, scale_max = sm, multi_scale = False, avg = True)#, scale_max = 0.1)
print(sm, fthin)
...
if not chckpt:
    ...
    posterior_samples = train_bayesian_flow(model, unravel_fn, scale_max = sm, num_warmup = nt, num_samples = ns, target_accept = 0.8, num_chains = nc, checkpoint_file = f"__run__/checkpoint_{out}.pkl", posterior_file = f"__run__/posterior_checkpoint_{out}_3.pkl", nbatch=100)#, anealing = False)#True)
